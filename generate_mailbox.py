...
def create_extruded_element(
    ...
    product_class="IfcBuildingElementProxy",
    ...
    solid = model.create_entity(
        ...
        SweptArea=profile,
        # Lokales Z der Extrusion wird auf globale Y gedreht.
        Position=axis2placement_3d(model, z_dir=(0.0, 1.0, 0.0), x_dir=(1.0, 0.0, 0.0)),
    ...
    # Drehung: zuerst globale Y um 180°, danach um globale Z um 180° (Resultat: X bleibt, Y invertiert, Z invertiert)
    product.ObjectPlacement = local_placement(
        model,
        relative_to=placement_rel_to,
        location=(0.0, 0.0, 0.0),  # Einfügepunkt im Ursprung belassen
        z_dir=(0.0, 0.0, -1.0),
        x_dir=(1.0, 0.0, 0.0),
    )
    product.Representation = representation

    # 7. Zuordnung: räumlich oder als Aggregat
    ...
    if aggregate_parent:
        ifcopenshell.api.run(
            ...
            model,
            relating_object=aggregate_parent,
            products=[product],
        )
...
def inset_rectangle(points, offset):
    """Shrink an axis-aligned rectangle by offset on all sides."""
...
def generate_metal_plate_ifc(dateiname="metal_plate.ifc", thickness=0.002):
    ...
    storey = ifcopenshell.api.run("root.create_entity", model, ifc_class="IfcBuildingStorey", name="Groundfloor")
    ...
    # Top-Level Produkt "Bierkasten" als Furnishing Element
    bierkasten = model.create_entity(
        "IfcFurnishingElement",
        ifcopenshell.guid.new(),
        Name="Bierkasten",
    )
    bierkasten.ObjectPlacement = local_placement(model, relative_to=storey.ObjectPlacement)
    ifcopenshell.api.run(
        "spatial.assign_container",
        model,
        products=[bierkasten],
    ...
    # Einlagen in den Öffnungen mit 1mm Inset
    inset_offset = 0.001  # 1mm
    insert_names = {
        1: "Schild Keine Werbung",
        2: "Schild Beschriftung",
        3: "Einwurfklappe",
    }
    for idx, hole in enumerate(inner_holes, start=1):
        if hole["type"] != "polyline":
            continue
        inset_points = inset_rectangle(hole["points"], inset_offset)
        create_extruded_element(
            model,
            bierkasten.ObjectPlacement,
            insert_names.get(idx, f"Einlage_{idx}"),
            inset_points,
            thickness,
            arc_points=[],
            inner_profiles_data=[],
            product_class="IfcPlate",
            aggregate_parent=bierkasten,
        )

    # Rahmen um die Platte (rechteckig, ohne Radien)
    frame_outer_offset = 0.018  # 18mm nach außen
    frame_inner_offset = 0.003  # 3mm nach innen
    xmin, xmax, ymin, ymax = bounding_rectangle(outer_points)
    frame_outer = [
    ...
    # Innere Linie 3mm nach außen versetzt (nicht nach innen)
    frame_inner = [
    ...
    create_extruded_element(
        ...
        "Briefkastenrahmen",
        frame_outer,
        0.35,  # 0.35 m Extrusion
        arc_points=[],
        inner_profiles_data=[{"type": "polyline", "points": frame_inner, "closed": True}],
        product_class="IfcPlate",
        aggregate_parent=bierkasten,
    )
